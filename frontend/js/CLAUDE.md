...
- **`openFileModal()` router efter filendelse** — PDF renderes via en bundlet offline pdf.js-viewer (`frontend/vendor/pdfjs/viewer.html`), fordi Android WebView (Fully Kiosk) ikke har en indbygget PDF-renderer som desktop Chrome. Billeder går uden om modalen helt (lightbox). Alt WebView ikke kan vise (docx/xlsx/pptx/zip/ukendt) viser et fallback-skærmbillede (`#file-modal-fallback`) med download-knap i stedet for at forsøge en iframe der ville fejle stille. Tilføj nye filtyper til `INLINE_TEXT_EXT`/`IMAGE_EXT` i stedet for at ændre selve routing-logikken.
