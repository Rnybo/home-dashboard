...
**Ét dokument kan (og gør typisk i praksis) indeholde MANGE ugers ugebreve** — skolen genbruger ét løbende Google Docs-dokument for hele skoleåret i stedet for at lave et nyt pr. uge, med "Uge XX" som overskrift for hver ny sektion. `split_document_into_weeks(html)` splitter dokumentet i sektioner ved disse overskrifter (kun et afsnit der UDELUKKENDE siger "Uge XX", matchet via `SECTION_HEADING_RE` — en sætning der blot nævner en uge i forbifarten splitter IKKE). `parse_document(html)` parser hver sektion for sig til `{"week", "days", "body_text"}`. **Uden denne opsplitning blandede brødteksten (og skemaet, for dokumenter med flere tabeller) ALLE ugers indhold sammen** — det var netop symptomet der afslørede problemet ("jeg får tekst fra alle ugebrevene" i brødtekst-boksen). `_parse_table(table)` er den rå tabel-til-dage-parsing, genbrugt pr. sektion.

- **Kerne-funktionen `_sync_core(client, calendar_tag, doc_url, anchor_date)` looper over ALLE uger `parse_document()` finder** — ikke kun én. Returnerer `{"found": True, "weeks": [{"week":.., "year":.., "events_created":..}, ...], "events_created": <total>}`. Kilde-uafhængig med vilje: tager imod en allerede-bestemt kalender-tag + URL i stedet for selv at slå noget op. Både `sync_ugebrev_url()` (bruger-klik) og `sync_ugebrev()` (automatisk scanning) deler den uden at duplikere hente/parse/gem-logik.
...
- **`sync_ugebrev(client)`** — heller ingen parametre — er baggrundsscanneren (hver 6. time, `_ugebrev_sync_loop` i `main.py`) og "Synkroniser nu"-knappen i settings.html. Looper over ALLE børn, tjekker hver enkelts seneste opslag, og synkroniserer hvert barn med et matchende Google Docs-link til sin egen kalender — håndterer flere børn i forskellige klasser uden videre. Returnerer `{"found": False, "message": ...}` eller `{"found": True, "results": [{"child_name":.., "weeks":[...], "events_created":..}, ...]}` — **bemærk to niveauer af lister** (én liste over børn, med en indlejret liste over uger pr. barn). Frontend (`formatUgebrevSyncResult()` i `globals.js`, delt af `aula.js`/`calendar.js`) og settings.html's egen visning håndterer dette — kopiér IKKE den gamle flade `data.week`/`data.year`-antagelse fra tidligere versioner.
- **Brødtekst-udtræk**: hver sektions "body_text" = al paragraf-/overskriftstekst UDENFOR selve skematabellen, KUN inden for den sektion — typisk lærerens ugentlige huskere. Gemmes separat via `store.save_ugebrev_note()` (nøgle `"{calendar_tag}|{year}|{week}"`, egen lille JSON-fil `ugebrev_notes.json`, IKKE en del af `custom_events.json` — det er ikke et kalenderevent). Hentes af `GET /api/ugebrev/info` til info-ikonet i `frontend/js/skolekalender.js`. Ekskluderer korrekt tabelcelle-tekst via `tag.find_parent("table") is table`-tjek — test dette specifikt hvis du ændrer parsingen.
- **Ugenummer læses fra hver sektions egen overskrift** ("Uge XX"), ikke fra opslagsdatoen — men den dato (opslagets/beskedens tidsstempel) bruges som fælles "anker" til at vælge det rigtige ÅR for HVER uge for sig (`resolve_week_dates`), fordi et ugenummer alene er tvetydigt over årsskiftet (uge 1 delt i december hører til næste år). Testet med begge retninger af årsskifte-tvivlen.
- **Robusthed mod variation mellem klasser/skoler** (august 2026, efter fem konkrete forbedringer uden nogen ny bug-rapport at gå ud fra — proaktivt, baseret på hvad der plausibelt KUNNE variere): `SECTION_HEADING_RE` accepterer nu "Uge XX" fulgt af et skilletegn og valgfri ekstra tekst ("Uge 33 – Kragelundskolen"), ikke kun en bar linje — men afviser stadig en sætning der blot begynder med et ugenummer uden skilletegn ("Uge 33 var fantastisk..."), for ikke at fejlsplitte dokumentet. `_parse_table()` prøver alle tabeller i en sektion i rækkefølge (`parse_document()`), ikke kun den første — en note-tabel før selve skemaet springes nu over. `DAY_ABBREVIATIONS` udvider dag-matchet til at acceptere "Man"/"Tirs"/"Ons"/"Tors"/"Fre". `_parse_time_range()` respekterer nu et EKSPLICIT angivet sluttidspunkt i en celle (`"8.00-8.45"`) i stedet for altid at udlede sluttiden af næste rækkes start — retter forkert strakte aktiviteter hvis der er et hul i skemaet (fx et frikvarter uden sin egen række). `_find_calendar_tag_for_doc()` sammenligner nu dokument-ID (`_doc_id()`), ikke hele URL-strengen — Google kan variere query-parametre (`usp=sharing` vs. `usp=drive_link`) for samme dokument. Alle fem er regressionstestet mod BÅDE syntetiske edge-cases OG det oprindelige ægte eksempeldokument (identisk resultat: uge 33, 30 blokke).
- **Bevidst IKKE forsøgt endnu** (ingen konkrete eksempler at basere det på — ville være at gætte): helt anden tabel-orientering (dage som rækker i stedet for kolonner), datakilder ud over Google Docs (Sheets, PDF), udvidelse af `ICON_KEYWORDS` til andre klassetrins fagord (falder allerede gracefult tilbage til `DEFAULT_ICON` 📋, ikke en fejl). Udvid disse når/hvis et konkret dokument faktisk viser behovet.
- **Kildetabellens tidskolonne kan skrive et interval** (`"8.00-8.45"`, `"8-8.45"`, eller med en Unicode-dash i stedet for almindelig bindestreg — Google Docs' autokorrektur bytter den nogle gange ud) — `_parse_time_range()` bruger nu det EKSPLICITTE sluttidspunkt hvis angivet (se robusthed-punktet ovenfor), og en bar time uden minutter ("8") tolkes som hele timen ("08:00"). Uden bare start-parsingen ville hele rækken parses som ugyldig og forsvinde stille, hvilket i praksis fjernede en hel dags eneste indhold.
