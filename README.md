...
- **Ugebrev → Kalender** — Auto-detects the school's weekly schedule (shared as a Google Docs link inside an Aula "ugebrev" message, not a real attachment) and creates named, icon-tagged calendar events for a configured child. A "🎒 Tilføj til skolekalender" button appears directly on any message containing a Google Docs link, so you're never dependent on subject-line matching or background-scan timing. Includes a child-friendly "🎒 Skoledag" popup (per-child tab) showing just today's/this week's school schedule in large icon-first blocks — separate from the full week grid. Configure in Settings → Ugebrev → Kalender.
