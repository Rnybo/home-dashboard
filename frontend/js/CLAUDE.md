...
- **`_renderWeekNow()`s barn-fane-gren konsoliderer `source==="ugebrev"`-events til ÉT `_schoolSummary`-blok** ("🏫 Skole", spænder fra tidligste start til seneste slut den dag) i stedet for at vise hvert tidsblok (Frugt, Dansk, SFO, ...) enkeltvis — ellers oversvømmer et enkelt ugebrev en hel dags kolonne. Andre custom/Aula-events på samme dag er upåvirkede, kun `source==="ugebrev"` filtreres ud af `childEvents` og erstattes. `weekOffset` (0/1) mappes direkte til skolekalenderens `_scope` ('week'/'nextweek') — se `app.js::renderCalEvents()`s `_schoolSummary`-gren for selve renderingen og klik-håndteringen (`onclick="openSchoolCalendar(childId, scope)"`, IKKE den normale `openEvInfo()`).
...
- **Tre visninger: `day`/`week`/`nextweek`** — nødvendigt fordi et ugebrev typisk kommer inden indeværende uge er slut, så to ugers events kan være gemt samtidig. Scope-knapper matches via `data-scope`-attribut, IKKE knappens tekstindhold (en tidligere version sammenlignede `textContent`, hvilket ville være blevet skørt med en tredje knap). `_isoWeekNumber()` viser kun ugenummeret i titlen som orientering — uden det er "Denne uge" og "Næste uge" visuelt umulige at skelne når begge (endnu) er tomme.
- **ℹ️-ikonet viser ugebrevets brødtekst** (`GET /api/ugebrev/info?calendar=..&week=..&year=..`) — `renderSchoolCalendar()` beregner `weekMonday`/ugenummer for ALLE tre scopes, ikke kun week/nextweek, og sætter dem som `data-week`/`data-year` på `#school-cal-info-box`, så ikonet virker konsekvent uanset hvilken visning der er aktiv. Toggler åben/lukket ved klik.
- **`openSchoolCalendar(childId, initialScope)`s 2. parameter er valgfri** (default `'day'`) — 🎒-knappen på barnefanen kalder den uden, ugekalenderens "🏫 Skole"-blok (`calendar.js`) kalder den MED (`'week'`/`'nextweek'`, udledt af `weekOffset`). Fjern ikke default-værdien, det ville ødelægge 🎒-knappens nuværende "åbn til i dag"-adfærd.
