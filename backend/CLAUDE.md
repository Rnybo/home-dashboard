...
**Dokumentet hentes som `.docx`, ikke `.html` (skiftet september 2026)** — `fetch_doc_docx_bytes()` (tidligere `fetch_doc_html()`). Googles anonyme `export?format=html` blev observeret at hænge UDEN noget svar overhovedet for en ikke-browser-klient (bekræftet reproducerbart med både `requests` og `curl`, med og uden browser-agtig User-Agent), mens `export?format=docx`/`export?format=txt` af SAMME dokument svarer på under et sekund — årsagen menes at være en anti-scraping-foranstaltning hos Google specifikt mod HTML-export, ikke et lokalt netværksproblem (den almindelige `/edit`-side og de andre exportformater er upåvirkede). `.docx` blev valgt frem for `.txt` fordi kun `.docx` bevarer tabelstrukturen. `parse_document()`/`split_document_into_weeks()`/`_parse_table()` bruger derfor `python-docx` (`Document`, `Table`, `Paragraph`) i stedet for BeautifulSoup — se `_iter_block_items()` for hvordan afsnit og tabeller besøges i dokumentets faktiske rækkefølge (python-docx's egne `.paragraphs`/`.tables` er separate flade lister uden indbyrdes rækkefølge, hvilket ville gøre det umuligt at afgøre hvilken tabel der hører til hvilken "Uge XX"-overskrift). BeautifulSoup bruges stadig andetsteds i filen til selve Aula-opslagets/beskedens HTML-indhold (gren 3, `plain_text`) — det er kun Google Docs-hentningen der er ændret.

**Fortsat samme kontrakt/formål som før (kun det underliggende dokumentformat er skiftet) — stadig brugt af `sync_ugebrev_url()`/`_sync_core()` (den manuelle knap-flow) og af selve tabel-parsingen i gren 1 ovenfor:**
...
- `_parse_table()` — dag-headers matches uafhængigt af versaler/kolon, `DAY_ABBREVIATIONS` accepterer forkortelser ("Man"/"Tirs"/osv.), dubletter af samme dagnavn kollapses til første forekomst (inkl. python-docx's egen adfærd med at gentage samme celle-reference for hver kolonne en vandret sammenlagt celle spænder over), `_parse_time_range()` respekterer et eksplicit sluttidspunkt i en celle frem for altid at udlede det af næste rækkes start.
