...
- **`_renderWeekNow()`s barn-fane-gren konsoliderer `source==="ugebrev"`-events til ÉT `_schoolSummary`-blok** ("🏫 Skole", spænder fra tidligste start til seneste slut den dag) i stedet for at vise hvert tidsblok (Frugt, Dansk, SFO, ...) enkeltvis — ellers oversvømmer et enkelt ugebrev en hel dags kolonne. Andre custom/Aula-events på samme dag er upåvirkede, kun `source==="ugebrev"` filtreres ud af `childEvents` og erstattes. **Gælder kun tabel-baserede (tidsbestemte) ugebrev-events** — billed-baserede events (`sfo_ugebrev`/`ugebrev_billede`, se `backend/CLAUDE.md`s `ugebrev.py`-sektion) er `allDay: true` og går derfor slet ikke gennem `rawChildEvents` (som filtrerer `!e.allDay`); de vises i stedet som almindelige heldags-badges i dag-headeren, samme sted som fx feriemarkeringer — bevidst IKKE konsolideret til "🏫 Skole"-blokken. `weekOffset` (den eksisterende globale ugenavigations-variabel, kan gå både frem og tilbage via `changeWeek()`) sendes DIREKTE videre som skolekalenderens `_weekOffset` — ingen oversættelse imellem de to, de bruger samme talskala. Se `app.js::renderCalEvents()`s `_schoolSummary`-gren for selve renderingen og klik-håndteringen (`onclick="openSchoolCalendar(childId, scope, weekOffset)"`, IKKE den normale `openEvInfo()`).
- **`loadOverview()` trigger en throttlet (30 min, `localStorage`-nøgle `ugebrev_auto_sync_ts`) baggrunds-sync via `_maybeTriggerUgebrevSync()`** når et opslag i det netop hentede feed har "ugebrev"/"ugeplan" i titlen — fire-and-forget `POST /api/ugebrev/sync`, ingen ventende UI-tilstand. Erstatter at skulle vente op til 6 timer på `_ugebrev_sync_loop` (`backend/main.py`) eller klikke "Synkroniser nu" manuelt i settings.html. Billigt at kalde ofte fordi backend selv kun gensynkroniserer manglende/nærmeste uger (se `_should_resync` i `backend/CLAUDE.md`).

## `skolekalender.js`

Børnevenlig "hvad skal jeg i dag/denne uge"-modal, uafhængig af den komplekse uge-grid. Åbnes via 🎒-knappen på et barns fane (se `globals.js`'s child-tabs-render). Viser events fra **tre kilder** (`source` i `["ugebrev", "sfo_ugebrev", "ugebrev_billede"]`) fra `/api/custom-events`, matchet på `calendar === "cal-child-<id>"` — ingen ny backend-kode, ren client-side filtrering af eksisterende data (se `backend/CLAUDE.md`'s `ugebrev.py`-sektion for hvordan de events opstår). Viste oprindeligt kun `"ugebrev"` — udvidet så billed-/SFO-baserede ugeplaner (heldagsevents, ingen tabel) også vises her, ikke kun i den almindelige kalenders dag-header-badges.

- **`_schoolCalMode` (`'child'`/`'sfo'`, tilføjet september 2026)** — samme modal, to indgange: `openSchoolCalendar(childId, ...)` (mode `'child'`, som hidtil) og `openSfoCalendar(...)` (mode `'sfo'`, ny "🏠 SFO"-fane i `globals.js`, IKKE en rigtig fane der skifter visning, samme klik-mønster som 🎒-ikonet). I `'sfo'`-mode filtrerer `_renderSchoolCalendarBody()` på `source === "sfo_ugebrev"` på tværs af ALLE børns `calendar_tag`, og DEDUPLIKERER på `(start, end, title)` — samme SFO-ugeplan bliver i praksis synkroniseret separat pr. barn med identisk indhold (bekræftet: 32 unikke kombinationer ud af 64 rå events for to børn), så uden dedup ville hver aktivitet vises dobbelt. ℹ️-ikonets `calendar_tag`-opslag læses fra et nyt `#school-cal-info-box`'s `data-calTag`-attribut (sat i `_renderSchoolCalendarBody()`) i stedet for altid at genberegne `'cal-child-' + _schoolCalChildId` — i `'sfo'`-mode er der jo intet enkelt barn at udlede den fra.

- **Titel-parsing forudsætter formatet `"{ikon} {tekst}"`** (splitter på det FØRSTE mellemrum) — det er et kontrakt med `backend/ugebrev.py`s `build_events()`/`_build_events_from_days_dict()`. Ændres formatet ét sted, skal det ændres begge steder.
- **`ev.allDay` skjuler det kunstige klokkeslæt** — billed-/SFO-events har intet rigtigt tidspunkt (kun dag+aktivitet), så et allDay-event viser ingen tid i stedet for det ellers vildledende "00:00" fra `ev.start`.
...
- **ℹ️-ikonet viser ugebrevets brødtekst** (`GET /api/ugebrev/info?calendar=..&week=..&year=..`) — `renderSchoolCalendar()` beregner `weekMonday`/ugenummer for BEGGE scopes (inkl. `day`), og sætter dem som `data-week`/`data-year` på `#school-cal-info-box`, så ikonet altid matcher den viste uge, uanset navigation. Toggler åben/lukket ved klik. **Boksen har sin egen `max-height`+interne scroll og et eksplicit luk-kryds** (`toggleSchoolCalInfo()` bygger `.info-box-head`/`.info-box-text` som separate elementer via `innerHTML`, ikke `textContent`) — rettet efter at lange ugebrev-noter (op til ~2000 tegn løbende prosa, ikke bare en kort huske-sætning) uden dette pressede resten af dialogen, inkl. selve luk-krydset i headeren, ud af syne uden nogen måde at scrolle eller lukke igen.
