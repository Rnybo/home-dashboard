...
**"Ugebrev" er i praksis observeret som et Aula ****opslag**** ("Opslag", delt med hele klassen via `posts.getAllPosts`), IKKE en privat besked.** Modulet blev oprindeligt bygget udelukkende omkring beskedtråde (`messaging.getThreads`/`getMessagesForThread`) — helt fejlfrit kode, men mod den forkerte datamodel, så featuren aldrig fandt noget indhold i praksis. `sync_ugebrev()` prøver derfor **opslag først** (`client.get_posts()`, matcher det første med et Google Docs-link i `content.html`/`content.text`), og falder kun tilbage til emne-matchede beskedtråde hvis intet opslag matcher. Antag ikke at fremtidigt indhold nødvendigvis er det ene eller det andet — hold begge veje vedlige.

Google Docs-linket (**ikke** en rigtig vedhæftning — skolen deler et delt dokument) hentes via `.../export?format=html` (kræver "alle med link kan se", ingen login), og parses som en dag×tidsblok-tabel til kalenderevents for et valgt barn (`UGEBREV_CHILD_ID`/`UGEBREV_ENABLED` i `.env`, sat via settings.html).

- **Kerne-funktionen `_sync_core(client, child_id, doc_url, anchor_date)` er bevidst kilde-uafhængig** — den tager imod en allerede-udtrukket URL + ankerdato i stedet for selv at slå en tråd/opslag op. Det er præcis derfor opslag/besked-forvirringen ovenfor var hurtig at rette: kun *hvor URL'en kommer fra* skulle ændres (`sync_ugebrev()`), ikke selve hente/parse/gem-logikken. Enhver fremtidig tredje kilde (kommentar, en anden slags opslag) kan tilsluttes ved bare at finde en URL og en dato, uden at røre `_sync_core`.
- **`sync_ugebrev_url(client, child_id, doc_url, anchor_date_str)`** er den variant frontend kalder direkte — bruges af "🎒 Tilføj til skolekalender"-knappen i **både** `aula.js`s besked-modal og `calendar.js`s opslags-modal (`openPost()`). Frontend udtrækker selv doc_url fra det indhold der allerede vises (`m.text.html` hhv. `p.content.html`) — ingen server-side tråd/opslag-genopslag nødvendig, hvilket også er hvorfor denne vej var nem at teste: mock kun `fetch_doc_html`, ingen `client`-mocking krævet.
- **Ugenummer læses fra dokumentets tekst** ("Uge XX"), ikke fra opslags-/beskeddatoen — men den dato bruges som "anker" til at vælge det rigtige ÅR for det ugenummer (`resolve_week_dates`), fordi et ugenummer alene er tvetydigt over årsskiftet (uge 1 sendt i december hører til næste år). Testet med begge retninger af årsskifte-tvivlen.
