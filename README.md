...
- **Ugebrev → Kalender** — Auto-detects the school's weekly schedule (shared as a Google Docs link inside an Aula *post*, shared class-wide — not a private message, and not a real attachment) and creates named, icon-tagged calendar events for a configured child. A "🎒 Tilføj til skolekalender" button appears directly on any post or message containing a Google Docs link, so you're never dependent on subject-line matching or background-scan timing. View any week's schedule in a child-friendly "🎒 Skoledag" popup (per-child tab, "I dag"/"Denne uge"/"Næste uge") — large icon-first blocks, separate from the full week grid. Configure in Settings → Ugebrev → Kalender.
